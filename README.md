...
Это мои настройки для сборки NixOS, включая настройки окружения Hyprland. Предназначена в первую очередь на платформы с видеосистемами Intel.
...
- https://github.com/mahaveergurjar/Hyprlock-Dots - настроки экрана блокировки Hyprlock. Там представлены несколько вариантов оформления, я немного модифицировал один из них, но там есть и другие варианты. Большое спасибо за труд этому человеку.
...
```
Шпаргалка `install_script` поможет не потеняться при установке системы NixOS. Если вы уже поставили Nixos, команда будет выглядеть несколько иначе, так как нужно сперва достучаться до `bash`:
```
cd nix-hyprland/.nix && /etc/profiles/per-user/username/bin/bash replace-username.sh
```
Не забудьте вместо username вписать имя вашего пользователя.
